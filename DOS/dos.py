...
try:
    ...
    trange = range
"""
区分自旋和非自旋，可以获取指定原子，指定轨道的态密度（t, s, p, d, tot）
"""
...
if __name__ == '__main__':
    ...
    parser.add_argument('-f', '--filename', type=str, default='DOSCAR-spin', help='DOSCAR name d:DOSCAR')
    parser.add_argument('-e', '--efermi', type=float, help='efermi level')
    parser.add_argument('-o', '--orbit', type=list, default=['t'], help='which orbit you choose, d:totaldos')
    parser.add_argument('-a', '--atomposition', help='POSCAR atom info')
    parser.add_argument('-p', '--peratom', default=1, help='DOS average to each atom, d:True')
    ...
    dos = DOS(FLAGS.filename, efermi=FLAGS.efermi, peratom=FLAGS.peratom)
    dos.get_orbit(FLAGS.orbit, atomlist=at, wri=True, tp=FLAGS.writetype)
